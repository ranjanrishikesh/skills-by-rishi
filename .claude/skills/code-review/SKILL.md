...
description: >
  ...
  exists. Runs typecheck, lint and build as a blocking gate first, then a
  rubric decides which review dimensions apply to this change set (CLAUDE.md
  compliance, bugs, truth, discoverability, interface, keyword contract, voice,
  reuse), then launches one parallel Sonnet reviewer per selected dimension.
  Interface renders the page in headless Chrome and checks appearance,
  behaviour, readability and mobile; Discoverability adds a Lighthouse audit.
  Where no browser can be attached, those two run in container mode instead:
  everything else reviews normally and the receipt records what nobody rendered.
  Findings are scored 0-100 for confidence, anything under 80 is dropped, then
  survivors are tiered Critical / High / Low / Minor. Critical and High are
  fixed by Opus 5 in the same pass; Low and Minor are not fixed, they are
  reported with a recommended fix each for the founder to accept or decline.
  Everything is reported via ReportFindings and repeated in the reply.
  Reviewers get the diff inline
  and a tool-call budget, and reason from reading rather than building test
  harnesses. Fixes are not re-reviewed:
  the model tier is the quality control. Scope is incremental: commits a previous
  run already reviewed are recorded and skipped, so a run covers only the
  unreviewed commits plus the working tree (staged, unstaged, untracked). Pass
  "full" to review the whole branch again. Does not touch GitHub and posts
  nothing.
argument-hint: "[optional: base branch, default origin/main] [full]"
disable-model-invocation: false
allowed-tools: Bash(git diff:*), Bash(git status:*), Bash(git merge-base:*), Bash(git ls-files:*), Bash(git fetch:*), Bash(git rev-parse:*), Bash(pnpm typecheck:*), Bash(pnpm lint:*), Bash(pnpm build:*), Bash(pnpm test:*), Bash(pnpm links:*), Bash(pnpm install:*), Bash(pnpm dev:*), Bash(lsof:*), Bash(kill:*), Bash(pkill:*), Bash(curl:*), Bash(node .claude/hooks/review-receipt.mjs:*), Task, Agent, Read, Edit, Grep, Glob, WebSearch, mcp__chrome-devtools__*
---
...
The base branch is `origin/main` unless the invocation names another.
...
**If the invocation contains the word `full`, append `--full` to the `scope` call in (b) and review the whole branch.** Anything else, including no argument at all, runs incrementally.
   ...
   a. `git fetch origin <base>` so the base ref is not stale.
...
```
node .claude/hooks/review-receipt.mjs scope <base>
```
...
Run these in order and stop at the first failure. Cheapest first, so a type error does not cost a full build.

```
pnpm typecheck   # tsc --noEmit
pnpm lint
pnpm build
pnpm test        # schema + rendered-output assertions; needs the build above
pnpm links       # every internal link, read from the built HTML
```
...
That is not caution, it is arithmetic. These five catch an entire class of defect deterministically, with zero false positives, for the cost of one command. Launching a fleet of language models at a tree that does not compile spends far more to find less, and buries the one finding that matters under a fleet's worth of speculation about code that was never going to run.

`pnpm test` and `pnpm links` run AFTER `pnpm build` and not before, because both read `.next/server/app/**/*.html`. That is the point of them: they check what shipped rather than what the source looks like, which is how a quoted `"href":` key went unchecked on every hard-coded page while the checker reported green. Both refuse to run against a stale build rather than reporting on the previous commit.

There is no CI at this point, because there is no PR. Nothing else runs these. If this skill does not, nothing does.

Two conditions to handle rather than fail on:

- **Dependencies not installed.** If `node_modules` is absent, run `pnpm install` once, then continue. Do not treat a missing install as a gate failure.
...
**Read `references/rubric.md` and follow it.** It classifies the changed files, decides which dimensions apply, sets the render gate, measures the size, and prints the plan.
...
## 5. Gather CLAUDE.md paths

Use a Haiku agent to return a list of file paths to (but not the contents of) any relevant CLAUDE.md files: the root file, plus any in directories the change set touched.

## 6. Summarise the change

Use a Haiku agent to read the change set and return a summary.

## 7. Start the dev server, only if the rubric said to

If the render gate qualified Interface or Discoverability, start **one** dev server now and note its base URL. Check whether one is already running first, and if so use it and do not stop it later. If neither qualified, skip this step.

Check for the browser before starting anything. If the `mcp__chrome-devtools__*` tools are not available, no dev server will help: **do not start one**, do not launch Interface, and tell Discoverability to run Passes A and B and skip Pass C. Every other dimension runs normally, and step 14 stamps in container mode so the receipt says what was not looked at.

## 8. Launch the reviewers

Launch one parallel agent per dimension **the rubric selected**, not per entry in the list below.

**Every reviewing agent runs on Sonnet 5 (`claude-sonnet-5`). No exceptions.** Reviewing is a wide, parallel, read-heavy job: many agents run at once, each reads a lot and returns a short list. Sonnet is the right tier for that shape, and holding every dimension to one model keeps their findings comparable, which is what makes the tier normalisation in step 11 mean anything. Do not silently upgrade a dimension because it feels harder.
...
- Give it the CLAUDE.md paths from step 5.
...
<!-- BEGIN REVIEW DIMENSIONS: add or remove entries here. One agent is launched per selected entry. To add a dimension, write a brief in references/dimensions/ following the structure of the existing ones, add a line here with a budget, and add a trigger row to references/rubric.md. Describe the roster by name rather than by count, so adding one does not leave a stale number behind. -->

   a. CLAUDE.md compliance: `references/dimensions/01-claude-md-compliance.md` (budget: 15)
   b. Bugs: `references/dimensions/02-bugs.md` (budget: 30)
   c. Truth: `references/dimensions/03-truth.md` (budget: 25)
   d. Discoverability: `references/dimensions/04-discoverability.md` (budget: 25)
   e. Interface: `references/dimensions/05-interface.md` (budget: 30)
   f. Keyword contract: `references/dimensions/06-keyword-contract.md` (budget: 20)
   g. Voice: `references/dimensions/07-voice.md` (budget: 20)
   h. Reuse: `references/dimensions/08-reuse.md` (budget: 25)

<!-- END REVIEW DIMENSIONS -->

Budgets are guidance for the agent, not a hard cut-off enforced by the harness. Bugs and Interface get more because one reads outward through callers and the other drives a browser. CLAUDE.md compliance gets least because it is quote-matching against a single file.

Paths are relative to this skill's directory. Shared context lives in `references/routes.md` (change set to URL mapping) and `references/severity.md` (tiers and fix authority). The voice sources of record live in `references/voice/`.

Two structures are deliberate and must not be flattened:
- **Bugs** runs a shallow diff-only pass, then a deep pass reading callers and callees, then a dependency pass when `package.json` or `pnpm-lock.yaml` changed. Do not split it into separate agents and do not let it collapse into one blended pass.
...
For each issue from step 8, launch a parallel Haiku agent that takes the change set, the issue description, and the CLAUDE.md paths from step 5, and returns a confidence score. For issues flagged on CLAUDE.md grounds, the agent must verify the CLAUDE.md actually calls out that issue specifically. Give this rubric to the agent verbatim:
...
**Every agent you spawn to apply a fix runs on Opus 5 (`claude-opus-5`).** This is the deliberate inverse of step 8, and it is what makes the next sentence safe.

**Fixes are not re-reviewed.** One pass: find, fix, ship. The model tier is the quality control, not a second reading. Finding a defect is recognition and parallelises well, so it goes to a wide fleet of Sonnet. Applying a fix is a change to working code where a wrong edit is worse than the finding it closed, so it goes to Opus, and Opus's output is trusted. Sending an Opus fix back to a Sonnet reviewer would be asking a weaker model to second-guess a stronger one, and it is what turned this loop into something that never terminated.

**Fix every CRITICAL and HIGH finding. Do not auto-fix LOW or MINOR.** Those are reported instead, each with a concrete recommended fix and a take-it-or-leave-it call, per `references/severity.md`. The point of the split is that a Minor fix spends the founder's attention on something a reasonable engineer could have declined, and buries the fixes that mattered inside a larger diff.

There is **one carve-out** to the fixing:

> **Factual claims about pricing, dates, clients, team size, or anything else only the founder can know are never fixed automatically.** Stop and ask. This is not a capability limit, it is an authority limit: a stronger model still does not know what agentclaw charges or which clients exist. Getting this wrong writes a confident falsehood onto a live page. Everything else, including judgment calls about abstractions, naming, structure and taste, is Opus's to make.

Then, without exception:

1. **Re-run the deterministic gate from step 3.** This is not a re-review. It is the same five commands: deterministic, zero false positives. It exists so a fix that breaks the build, the link graph or an invariant cannot ship. The tests matter most here, because a fix applied to rendering is exactly the kind of change that silently drops a `target`, a canonical or a utm parameter. If it fails, fix that too and run it again.
...
Report with the **`ReportFindings` tool**, in the tier order from step 11, so findings render in the host UI. Pass an empty `findings` array if nothing survived. Every finding needs a repo-relative `file`, the 1-indexed `line`, a one-sentence `summary`, and a concrete `failure_scenario` with specific inputs or state leading to the wrong outcome. Set `outcome` to `fixed` on the Critical and High findings step 12 fixed, and leave it unset on the Low and Minor ones, which were not.

**Then write the same tier-ordered list into the reply as well.** The tool call is the structured record; it is not a guaranteed presentation. On 2026-08-02 a review reported seven findings through the tool alone and the reader saw an empty response, because the panel did not render on their client. Duplication is cheap; an invisible review is worthless.
...
4. Anything escalated under the founder carve-out, stated as an open question with options and a recommendation.
...
```
node .claude/hooks/review-receipt.mjs stamp
```
...
2. Record the waiver, so the audit trail says what actually happened:
   ```
   node .claude/hooks/review-receipt.mjs waive "<the user's exact words>"
   ```
...
- **Anything a linter, typechecker, or compiler would catch.** The gate in step 3 already ran all three and they passed, so a reviewer reporting one of these is either wrong or looking outside the change set.
...
- Cite every finding with a `file` and `line` a reader can click. Do not construct GitHub blob URLs: the change set is local and may be unpushed, so no such URL exists.
